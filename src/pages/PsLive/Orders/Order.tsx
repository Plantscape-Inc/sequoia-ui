    ...
    async function updateOrder(newOrderId: number) {
        ...
        try {
            const data: Order = await (
                await fetch(`${API_URL}/order/${newOrderId}`)
            ).json();

            setOrder(data);
            setTempOrder(data);

            const billingRes = await (
                await fetch(`${API_URL}/address/${data.billing_address.id}`)
            ).json();
            setBillingAddress(billingRes);

            const shippingRes = await (
                await fetch(`${API_URL}/address/${data.shipping_address.id}`)
            ).json();
    ...
    const handleUpdate = async () => {
        ...
        try {
            const payload = {
                ...tempOrder,
                billto:
                    typeof tempOrder.billing_address === "object"
                        ? tempOrder.billing_address.id
                        : tempOrder.billing_address,
                accountlocation:
                    typeof tempOrder.shipping_address === "object"
                    ...
                    <div className="mt-6 grid grid-cols-4 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                id="entrydate"
                                type="date"
                                className="w-full rounded-lg border border-gray-300 bg-gray-50 p-2 text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
                                value={tempOrder.entrydate ? tempOrder.entrydate.split("T")[0] : ""}
                                onChange={(e) =>
                                    setTempOrder({
                                        ...tempOrder,
                                        entrydate: e.target.value || null,
                                    })
                                }
                            />
                        </div>
                        ...
                        <div>
                            <Label htmlFor="total">Total Mintues</Label>
                            <input
                                id="total"
                                type="number"
                                className="w-full rounded-lg border border-gray-300 bg-gray-50 p-2 text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
