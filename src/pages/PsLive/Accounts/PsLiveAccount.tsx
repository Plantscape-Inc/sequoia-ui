...
export default function AccountEditor() {
  ...
  async function updateAccount(newAccountId: string) {
    ...
    setLoading(true);
    try {
      const data: Account = await (
        await fetch(`${API_URL}/account/${newAccountId}`)
      ).json();
      setAccount(data);
      setTempAccount(data);

      if (data) {
        const addressResponse = await (
          await fetch(`${API_URL}/address/${data.address}`)
        ).json();
        ...
        if (data.billtoaddress === data.address) {
          ...
          const billToAddressResponse = await (
            await fetch(`${API_URL}/address/${data.billtoaddress}`)
          ).json();
