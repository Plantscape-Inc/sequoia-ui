...
export default function AccountLocationItemDisplay({ item }: EditableAccountLocationItemRowProps) {
    const API_URL = import.meta.env.VITE_PSLIVE_URL;
    const [localItem, setLocalItem] = useState<AccountLocationItem>(item);

    const handleFieldChange = (field: keyof AccountLocationItem, value: string | number) => {
    ...
    return (
        <TableRow className="bg-gray-50 dark:bg-gray-700 mt-32">
            <TableCell>
                {localItem !== item && (
